...
if (0 !== $key && null !== ($query = \get($_GET, $key))) {
    ...
    if ($file = \exist([
        ...
        \Hook::set('route.search', function ($content, $path, $query, $hash) use ($file, $files, $url) {
            if (null !== $content) {
                return $content;
            }
            ...
            $pager->query = \To::query([($state->x->search->key ?? 'query') => $name]);
            $GLOBALS['page'] = $page;
            $GLOBALS['pager'] = $pager = $pager->chunk($chunk, $part + 1);
